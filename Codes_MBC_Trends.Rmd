...
mbc_dataset = mbc_dataset_raw %>% 
    select(Subtype, Pretreated, Stratified, Age, N, wECOG,
         ORR, mPFS, mOS, Therapy_Class, Treatment_Size, Targeted, Targeted_m, Targeted_yn, Chemo_m, Hormonal)  

mbc_dataset = mbc_dataset %>%
  mutate(Treatment_Size = if_else(Treatment_Size == "4-Agent",
                        "3-Agent", Treatment_Size)) %>% 
mutate(across(where(is.character), as.factor)) %>% 
  mutate(Treatment_Size = factor(Treatment_Size, levels = 
...
rename(mOS_2 = mOS, 
       ...
       wECOG_2 = wECOG, 
       Size = Treatment_Size,
       Therapy = Therapy_Class)

mbc_dataset_mice_u =  bind_cols(mbc_dataset_mice_u, 
            mbc_dataset_raw %>% 
              select(wECOG, mPFS, mOS, PMID, Combo))

mbc_trend = mbc_dataset_mice_u %>% 
  ...
  mutate(Combo_other = fct_lump(Combo, n = 30)) %>% 
   select(Age, Subtype, Pretreated, wECOG, Therapy, Size, 
             ORR, mPFS, mOS, Combo,
          Combo_other,
          Targeted_m, Chemo_m, Hormonal, mOS_2, mPFS_2, wECOG_2) %>% 
   mutate(Size = forcats::fct_recode(Size,
    "1-agent" = "1-Agent",
    "2-agent" = "2-Agent",
    "3-agent" = "3-Agent"))
...
effectsize::interpret_r(-0.41, rules = "funder2019")
...
df_synergy = read_excel("Dataset_interaction.xlsx") %>% 
