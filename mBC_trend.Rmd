...
mk = mBC_labeled %>%
  filter(n_studies >= 5, 
         Combo_other != "Other") %>%
  mutate(Combo_label = reorder_within(Combo_label, mPFS_2, Subtype)) %>%
  ggplot(aes(Combo_label, mPFS_2, fill = Combo_other)) +
  geom_boxplot(color = "grey30", outlier.alpha = 0.5) +
  scale_x_reordered() +
  ...
  filter(n_studies >= 5, 
         Combo_other != "Other") %>%
  mutate(Combo_label = reorder_within(Combo_label, mOS_2, Subtype)) %>%
  ggplot(aes(Combo_label, mOS_2, fill = Combo_other)) +
  geom_boxplot(color = "grey30", outlier.alpha = 0.5) +
  scale_x_reordered() +
  ...
  guides(fill = "none") +
    ...
    cowplot::plot_grid(mk, km, nrow = 1)

ggsave(
      "cowplot_Regimen_trend.jpg",
      width = 14,                       
      height = 10,                      
      dpi = 600)
...
mk = mBC_labeled %>%
  filter(n_studies >= 5, 
         Combo_other != "Other") %>%
  mutate(Combo_label = reorder_within(Combo_label, mPFS_2, Subtype)) %>%
  ggplot(aes(Combo_label, mPFS_2, fill = Combo_other, color = Combo_other )) +
  ...
  filter(n_studies >= 5, 
         Combo_other != "Other") %>%
  mutate(Combo_label = reorder_within(Combo_label, mOS_2, Subtype)) %>%
  ggplot(aes(Combo_label, mOS_2, fill = Combo_other, color = Combo_other)) +
  geom_boxplot(show.legend = FALSE, 
                alpha = 0.2,
             #  size = 1.5, 
                color = "grey25", 
...
cowplot::plot_grid(PFS_row, sep_line, OS_row,
        ...
        ggsave(
      "cowplot_Size_Pretreated_comb.jpg",
      height = 8,                      
      width = 10,                       
      dpi = 600)
...
Synergy = read_excel("Clinical_interaction.xlsx") %>% 
  ...
  mutate(Combination = factor (Combination))
    ...
    min.segment.length = 0) +
      ...
      dpi = 600)
        ...
        effectsize::interpret_epsilon_squared(0.13)
        effectsize::interpret_rank_biserial(0.32)
